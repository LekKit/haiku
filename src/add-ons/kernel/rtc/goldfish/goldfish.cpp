...
GoldfishRealTimeClock::InitDriver(device_node* node, GoldfishRealTimeClock*& outDriver)
{
	...
	driver->fRegsArea.SetTo(map_physical_memory("Goldfish MMIO", regs, regsLen, B_ANY_KERNEL_ADDRESS, B_KERNEL_READ_AREA | B_KERNEL_WRITE_AREA, (void**)&driver->fRegs));
	if (!driver->fRegsArea.IsSet())
		return driver->fRegsArea.Get();

	rtc_set_hook(driver.Get());
